...
def test_virtual_in_udp_comment_follows_the_current_language():
    out = text_of(render_virtual_in_udp("L", "192.168.1.50", 7000, inputs()))
    assert 'Comment="generated by loxmatter"' in out

    i18n.set_language("de")
    out = text_of(render_virtual_in_udp("L", "192.168.1.50", 7000, inputs()))
    assert 'Comment="erzeugt von loxmatter"' in out


def test_virtual_out_comment_follows_the_current_language():
    out = text_of(
        render_virtual_out(
            "Lampe",
            "http://192.168.1.50:8080",
...
def test_system_templates_bridge_alive_title_and_comment_follow_the_current_language():
    viu, _ = render_system_templates("192.168.1.50", 7000, 8080)
    out = text_of(viu)
    assert 'Title="Bridge reachable"' in out
    assert 'Comment="Watchdog: toggles as long as the bridge is running"' in out

    i18n.set_language("de")
    viu, _ = render_system_templates("192.168.1.50", 7000, 8080)
    out = text_of(viu)
    assert 'Title="Bridge erreichbar"' in out
    assert 'Comment="Watchdog: toggelt, solange die Bridge laeuft"' in out


def test_system_templates_resync_title_follows_the_current_language():
    _, vo = render_system_templates("192.168.1.50", 7000, 8080)
    out = text_of(vo)
    assert 'Title="Resend all values"' in out

    i18n.set_language("de")
    _, vo = render_system_templates("192.168.1.50", 7000, 8080)
    out = text_of(vo)
    assert 'Title="Alle Werte neu senden"' in out
